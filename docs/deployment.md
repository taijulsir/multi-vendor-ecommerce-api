...
| `THROTTLE_TTL_MS` | Optional (defaults to `60000`) | The one shared rate-limit window, in milliseconds, every limit below counts requests against (`ThrottlerModule`, `src/app.module.ts`) | `60000` | No | `.env.production` |
| `THROTTLE_LIMIT` | Optional (defaults to `100`) | Global per-route-per-IP request ceiling for any route without its own override — each route gets its own independent bucket, this is not one counter shared across the whole API | `100` | No | `.env.production` |
| `THROTTLE_AUTH_LOGIN_LIMIT` | Optional (defaults to `5`) | Stricter override for `POST /auth/login` — the primary credential-stuffing/brute-force target | `5` | No | `.env.production` |
| `THROTTLE_AUTH_REGISTER_LIMIT` | Optional (defaults to `5`) | Stricter override for `POST /auth/register` — registration spam / Argon2id CPU-exhaustion target | `5` | No | `.env.production` |
| `THROTTLE_AUTH_REFRESH_LIMIT` | Optional (defaults to `10`) | Stricter override for `POST /auth/refresh` | `10` | No | `.env.production` |
| `THROTTLE_PAYMENTS_WEBHOOK_LIMIT` | Optional (defaults to `20`) | Stricter override for `POST /payments/webhook` — unauthenticated and, per [Section 18](#18-security-checklist), not signature-verified | `20` | No | `.env.production` |
| `THROTTLE_PRODUCT_IMAGE_UPLOAD_LIMIT` | Optional (defaults to `20`) | Stricter override for `POST /products/:productId/images` — real disk I/O + content-sniffing per call | `20` | No | `.env.production` |
| `THROTTLE_CHECKOUT_LIMIT` | Optional (defaults to `20`) | Stricter override for `POST /checkout` — a multi-table DB transaction per call | `20` | No | `.env.production` |

**No other environment variables exist in the current codebase** — this table is exhaustive against `env.validation.ts`'s `required` array plus the optional variables (`PORT`, `FILE_STORAGE_DIR`, and the `THROTTLE_*` rate-limit settings) it also validates when present.

**On the `THROTTLE_*` defaults above**: they are the values this application falls back to if an operator forgets to set them at all — a secure-by-default floor, not development-friendly numbers. `.env.example` deliberately sets much more generous values instead (the e2e suite alone makes 100+ register/login calls within a single window, since every e2e spec file shares one Redis-backed counter per route and the whole suite runs in well under a minute) — **do not copy `.env.example`'s throttle values into `.env.production`**; either omit them entirely (the strict defaults above apply automatically) or set them explicitly to the same numbers.
...
**`X-Forwarded-For` and rate limiting:** `main.ts` calls `app.set('trust proxy', 1)`, telling Express to trust exactly one reverse-proxy hop in front of it — matching this exact Nginx config, which is the only hop between the internet and the app. This makes `req.ip` (and therefore `ThrottlerGuard`'s default per-client tracking — see [Section 18](#18-security-checklist)) resolve to the real client's address from `X-Forwarded-For`, not Nginx's own `127.0.0.1`. If you ever add a second proxy/load balancer in front of this one (a CDN, another reverse proxy), `trust proxy` must become `2` (or the matching hop count) — leaving it at `1` behind two hops would let a client forge the address the app treats as "trusted," and each real client would still bucket correctly only by accident.
...
- [x] **Rate limiting** — implemented (`@nestjs/throttler`, `ThrottlerGuard` registered globally via `APP_GUARD`, Redis-backed storage via `RedisThrottlerStorage`/`src/throttler/`). A generous per-route-per-IP default applies everywhere (`THROTTLE_LIMIT`/`THROTTLE_TTL_MS`), with stricter overrides on `POST /auth/login`, `POST /auth/register`, `POST /auth/refresh`, `POST /payments/webhook`, `POST /products/:productId/images`, and `POST /checkout` — see the Environment Variables table above and `src/throttler/throttle-config.ts`. Requires [Section 11](#11-nginx)'s `X-Forwarded-For` header and `main.ts`'s matching `app.set('trust proxy', 1)` to be genuinely per-client, not per-Nginx-hop — verify post-deployment that repeated requests from different real clients are tracked independently (not sharing one bucket), and that a single client can actually trigger a `429` on a strict route (e.g. hammering `POST /auth/login` with a wrong password).
...
**Explicitly not implemented — do not claim these exist:** a CORS policy, a WAF, a secrets manager, webhook signature verification (no real payment gateway is integrated to verify signatures against). These are documented, disclosed gaps (see [`README.md`](../README.md#security) and [`docs/experience-level-readiness-audit.md`](experience-level-readiness-audit.md) Section 5), not oversights introduced by this deployment.
